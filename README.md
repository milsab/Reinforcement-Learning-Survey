...
To make exploitation happen, there is only one way that an agent can do which is acting greedy. It means that every time that an agent wants to use gathered-information (exploiting) it needs to act what it thinks is best. On the other hand, to make exploration happen, there are multiple ways that an agent can act to collect information such as exploring based on confidence or based on uncertainty. The following are the three most common exploration strategies that an agent can collect information to use them for future actions.

 1.  **$\epsilon$-greedy:** If an agent follows epsilon-greedy exploration strategy, it means that the agent with a probability of epsilon explores randomly and with a probability of 1 - $\epsilon$ exploits greedly. There is a slightly different version of $\epsilon$-greedy which it calls decying $\epsilon$-greedy. According to that, the value of $\epsilon$ will decrease gradullay. It means that the agent do more exploration at the beginning of learning in order to collect more information and then it starts to do more exploitaion based on the collected information while still do exploration but with lower probability of $\epsilon$ .
 2. **Softmax:** In this strategy, an agent explores based on current action-value estimates. Unlike $\epsilon$-greedy that the agent acts uniformly at random from the set of available actions, in softmax strategy, the agent takes more actions that have higher estimated values while it takes less actions that have lower estimated values.
